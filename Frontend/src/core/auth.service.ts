...
export class AuthService {
  ...
  me() {
    return this.http.get<{data:{user:User}}>(`${this.base}/me`).pipe(
      tap({
        next: (res)=> this.user$.next(res.data.user),
  ...
  login(email: string, password: string) {
    return this.http.post<{data:{user:User}}>(`${this.base}/login`, { email, password }).pipe(
  ...
  register(payload: {
    name: string; email: string; password: string; level: 1|2|3;
    studentProfile?: any;
  }) {
